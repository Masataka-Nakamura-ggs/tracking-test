/**
 * oneAccountSales.js - OneAccount Tracking Script (Cross-Domain Integrated)
 *
 * このスクリプトは、トラッキング仕様書に基づいて実装されています。
 * 1つのファイルで以下の2つの主要な機能を持ちます。
 * 1. oneAccountSales()関数による成果計測
 * 2. ページロード時に自動実行されるクロスドメイン対応機能
 *
 * @version 1.2.1 (commented)
 */
(function(window, document) {
     ...
     * scriptタグに `data-a8stoplog="1"` がある場合、ログ出力を停止します。
     * @returns {{info: Function, error: Function}} ログ出力用の関数を持つオブジェクト
     */
    const getLogger = () => {
        const scriptTag = document.querySelector('script[src*="oneAccountSales.js"], script[src*="SampleTrackingScript.js"]');
        const stopLog = scriptTag && scriptTag.dataset.a8stoplog === '1'; // data-a8stoplogは互換性のため維持
        return {
